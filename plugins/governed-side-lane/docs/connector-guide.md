...
| Run status or run report for one of the coordinator's own dispatched runs | The fixed `cm-services` MCP server's Gateway read tools | The coordinator provisions the same fixed user-global `cm-services` registration used by the other cm-services capabilities, and its own server-side grant file names the runs. | That the worker observes the exact granted tool names. `gateway-read` grants only `mcp__cm-services__gateway_run_status` and `mcp__cm-services__gateway_run_report`, and the server allows only the run IDs in the coordinator's `gateway_run_ids` grant file — a status read is never authority to cancel, retry, or change a run, and neither tool takes a URL, token, or other credential argument. |
