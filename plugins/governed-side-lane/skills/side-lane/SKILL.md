...
execute lanes do not render this allowlist. The grants name the server, so the
MCP server must be registered as exactly `gitnexus` / `codegraph`;
`check-capabilities` reports `name-mismatch` for a near-miss and the launch
gate refuses it.
