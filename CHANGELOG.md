...
- Resolve report lanes (`--report-only`/`--report-deliverable`) to the `standard` execute profile before the adapter argv is built, including routes a private table opted into `local-developer`; refuse an explicit `local-developer` selection beside a report flag at the CLI resolver and the Claude `launch`/`build_command` boundaries. Ordinary implementation lanes on opted-in local routes keep the wider profile unchanged.
- Add an explicitly configured local-developer execute profile for Claude-host lanes. It uses native shell access and the child’s registered MCP tools while preserving provider identity, report restrictions, strict review, and the cm-services proxy’s exact grants. Public and cloud defaults remain standard; private local routes opt in deliberately.
- Resolve inherited MCP inventory from the actual child configuration directory. Registration and successful local tool reads do not establish OAuth-cache parity; externally stored MCP OAuth credentials remain an explicit limitation.
- Admit typed MCP registration presence for execute recommendation with an unverified-presence label, without relabelling it authenticated access.
- Expose the recommender's request validation as one pure function, `validate_recommendation_request`, so a caller that cannot reach a route can still refuse a malformed request in the CLI's own wording and exit status. It performs the host, mode, connector-list, and capability-name checks that already ran before the first capability probe; the message text, ordering, and status are unchanged, and no host executable, MCP registration, credential store, or selector policy is read.
- Staff an execute recommendation for a typed MCP capability on the host's own matching rule rather than the exact name alone, so a Codex lane's inherited near-name code-graph registration (`gitnexus-local`) counts for `gitnexus` as its own evidence already did; Claude, Devin, and the capabilities whose grants embed the server name on every host stay exact. Review mode now drops every capability the canonical mapping delivers as an MCP server, including the per-run `aws-read`/`omniroute-read`, not only the user-scope subset.
- Hold the names-only registration scanner to the JSON its host would accept: a misspelled literal, an ungrammatical number, an invalid string escape, a non-object root, a non-object `mcpServers`, and trailing data after the top-level value are all refused instead of yielding names from a file the host never loads. Values are still discarded and no credential is materialized or logged.
- Read an `mcpServers` key as a registration container only at the two positions a host reads one at — the file's root object and a `projects.<path>` entry. A field spelled `mcpServers` inside a server definition (or in an array, or under any other key) is data: it no longer refuses the file when its value is not an object, and it no longer contributes a registration, so the inventory matches the host loader that reads a definition without inspecting what is inside it.
- Refuse a host MCP registration that exists but cannot be read or parsed instead of scanning it as an empty inventory. The gate and `check-capabilities` reported such a file as a missing connector (`unknown`, then "required capabilities unavailable"), an absence claim the evidence did not support; the worker-side registry reader already failed closed on the same path.
