...
- `check-capabilities` reports `gitnexus`/`codegraph` as `present` only when an
  MCP server is registered under exactly that name. A server whose name merely
  ...
  `name-mismatch` like any non-present state (#20).
